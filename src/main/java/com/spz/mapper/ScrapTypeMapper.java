...
public interface ScrapTypeMapper {
    @Insert("insert into scrap_type(name, create_time, update_time)  values (#{name}, #{createTime}, #{updateTime})")
    ...
    @Update("update scrap_type set name=#{name}, update_time=#{updateTime} where id=#{id}")
